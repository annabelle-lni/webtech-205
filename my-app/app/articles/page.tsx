...
export default async function ArticlesPage({ searchParams }: PageProps) {
  ...
  // Récupération des recettes avec filtre si recherche
  let query = supabase
    .from("recette")
  ...
  return (
    <main
      ...
      >  
        {/* Titre diff en fonction de la recherche */}
        <h1 className="text-[22px] font-bold mt-12 pt-8 mb-8">
          {searchQuery 
            ? <>Nos recettes pour <span className="font-bold text-[#f4a887] italic">"{searchQuery}"</span></> 
            : "Nos recettes"
          }
        </h1>
        ...
        {/*esthétique A REVOIR*/}
        {searchQuery && recettesWithPhotos.length === 0 && (
          <div className="mb-8">
            ...
            <Link 
              href="/articles" 
              className="text-[#f4a887] hover:underline mt-4 inline-block"
            >
        ...
        <div className="my-12 mx-auto grid grid-cols-[repeat(auto-fill,minmax(230px,1fr))] gap-8 items-start w-[calc(100%-80px)] max-w-[1100px] box-border justify-items-center">
            {recettesWithPhotos.length > 0 ? (
              ...
              // Message si aucune recette du tout (sans recherche)
